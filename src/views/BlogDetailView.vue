  ...
  <main class="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-20 lg:px-8 bg-black flex-1 w-full shadow-2xl">
    ...
    <article v-if="post" class="space-y-8">
      ...
      <header class="space-y-4">
        ...
        <!-- Post Title -->
        <h1 class="text-3xl font-extrabold tracking-tight text-white sm:text-4xl light:text-zinc-950">
          {{ post.meta.title }}
        </h1>
    ...
    <div v-else class="text-center py-20 border border-dashed border-zinc-800 rounded-2xl light:border-zinc-300">
      <h2 class="text-xl font-bold text-white light:text-zinc-900">Blog Post Not Found</h2>
      <p class="mt-2 text-zinc-400 light:text-zinc-650">The blog post you are looking for does not exist or has been moved.</p>
      <button 
        @click="goBack" 
