...
<template>
  <main class="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-20 lg:px-8 bg-black flex-1 w-full shadow-2xl">
    ...
    <article v-if="workItem" class="space-y-8">
      ...
      <header class="space-y-4">
        ...
        <div class="flex flex-wrap items-center gap-3 text-xs text-zinc-500 dark:text-zinc-400">
          ...
          <span v-if="workItem.meta.featured" class="px-2 py-0.5 rounded bg-zinc-100 text-zinc-800 font-medium dark:bg-zinc-900 dark:text-zinc-200">
            Featured
          </span>
        </div>
    ...
    <div v-else class="text-center py-20 border border-dashed border-zinc-300 rounded-2xl dark:border-zinc-800">
      <h2 class="text-xl font-bold text-zinc-900 dark:text-white">Work Entry Not Found</h2>
      <p class="mt-2 text-zinc-650 dark:text-zinc-400">The work entry you are looking for does not exist or has been moved.</p>
      <button 
        @click="goBack" 
