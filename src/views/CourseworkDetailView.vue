...
const { getCourseworkBySlug } = useCoursework()
...
const parsedHtml = computed(() => {
  if (!courseworkItem.value) return ''
  return marked.parse(courseworkItem.value.content || '')
})
...
<template>
  <main class="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-20 lg:px-8 bg-black flex-1 w-full shadow-2xl">
    ...
    <!-- Found State -->
    <article v-if="courseworkItem" class="space-y-8">
      
      <!-- Header Meta -->
      <header class="space-y-4">
        <!-- Date & Featured Tag -->
        <div class="flex flex-wrap items-center gap-3 text-xs text-zinc-400 light:text-zinc-500">
          <span v-if="courseworkItem.meta.date" class="flex items-center gap-1">
            <Calendar class="h-3.5 w-3.5" />
            {{ new Date(courseworkItem.meta.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) }}
          </span>
          <span v-if="courseworkItem.meta.featured" class="px-2 py-0.5 rounded bg-zinc-900 text-zinc-200 font-medium light:bg-zinc-100 light:text-zinc-800">
            Featured
          </span>
        </div>
        ...
        <h1 class="text-3xl font-extrabold tracking-tight text-white sm:text-4xl light:text-zinc-955">
          {{ courseworkItem.meta.title }}
        </h1>

        <!-- Description -->
        <p class="text-lg text-zinc-400 light:text-zinc-600 leading-relaxed italic">
          {{ courseworkItem.meta.description }}
        </p>

        <!-- Tags -->
        <div class="flex flex-wrap gap-2 pt-2">
          <span 
            v-for="tag in courseworkItem.meta.tags" 
            :key="tag"
    ...
    <div v-else class="text-center py-20 border border-dashed border-zinc-800 rounded-2xl light:border-zinc-300">
      <h2 class="text-xl font-bold text-white light:text-zinc-900">Coursework Not Found</h2>
      <p class="mt-2 text-zinc-400 light:text-zinc-650">The coursework entry you are looking for does not exist or has been moved.</p>
      <button 
        @click="goBack" 
        class="mt-6 inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-semibold text-zinc-950 shadow-sm hover:bg-zinc-100 light:bg-zinc-900 light:text-white light:hover:bg-zinc-800 transition-colors"
      >
        Go Home
      </button>
    </div>
